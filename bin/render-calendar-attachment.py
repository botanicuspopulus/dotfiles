...
def pretty_print_invitation(invitation):
    ...
    if invitation.method.value == 'REPLY':
        ...
        print("\u001b[48;1;37m", ("Event Invitation").center(CONTENT_WIDTH), "\u001b[0m",)
        print("\u001b[48;37m", (smart_truncate(title.strip(), 68)).center(CONTENT_WIDTH), "\u001b[0m",)
        print("\u001b[48m", " " * CONTENT_WIDTH, "\u001b[0m")
        # print("Event Name: %s" % title.strip())
        print("")
        print(f"Date/Time:   {when_str_of_start_end(start, end)}")
        print("Location:    %s" % (location or ('Microsoft Teams meeting' if 'Microsoft Teams meeting' in description else '<None>')))
        ...
        print("Invitees:")
        for i in invitees:
            print("  {person_string(i) if i else ''}")
